                    ...
                    <div class="col-md-12">
                        <form class="form-inline pull-right" id="search-form" onsubmit="return false;">
                            ...
                            <div class="form-group">
                                <input type="text" style="width: 160px;"  class="form-control" id="mac" name="mac" placeholder="请输入MAC地址">
                            </div>

                            <div class="input-group">
                                <input type="text" class="form-control" name="ssid" id="ssid" style="width: 145px;" placeholder="请输入ssid">
         ...
         $(window).ready(function () {
            ...
            $(document).on('click', '#server-choose-btn', function () { //  点击选择按钮
                ...
                if ( $(':checked') ) {
                    ...
                    parent.user.doSetRouter(mac_id,select_sid);
                    plugin.close();
                } else {
                    layer.alert('最少选择一个路由', {icon: 2, offset: '70px'});
                    return false;
                }
                    ...
                    success: function( obj ) {
                        ...
                        if (obj.total > 0) {
                            for (var i in obj.rows) {
                                html += '<tr id="server_content_'+obj.rows[i]['mac_id']+'" align="center">';
                                html += '<td >'+obj.rows[i]['mac_id']+'</td>';  //id
                                html += '<td>'+obj.rows[i]['mac']+'</td>';  //IP地址
                                html += '<td >'+obj.rows[i]['ssid']+'</td>'; //服务器IP地址
                                html += '<td >'+obj.rows[i]['expire_date']+'</td>';  //本地端口
                                html += '<td >'+obj.rows[i]['useFlag']+'</td>';  //服务器端口
                                ...
                                html += '<span style="display : inline-block; width: 100px; margin-bottom: 0px;overflow: hidden;height:22px;">';
                                    ...
                                    if(jQuery.inArray(obj.rows[i]['mac_id'], obj.rid)!=-1){
                             
                                        html += '<input type="checkbox" class="square-radio server" id="'+obj.rows[i]['mac_id']+'" name="server" value="'+obj.rows[i]['mac_id']+'" checked>&ensp;请选择';
                                    } else {

                                        html += '<input type="checkbox" class="square-radio server" id="'+obj.rows[i]['mac_id']+'" name="server" value="'+obj.rows[i]['mac_id']+'" >&ensp;请选择';
                                    }
